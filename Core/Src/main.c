...
TrafficState previous_state=YELLOW;
...
int main(void)
{
  ...
  while (1)
  {
	...
	case YELLOW:
		...
		if((HAL_GetTick()-stateStartTime)>2000){
	...
	case GREEN:
		...
		else if((HAL_GetTick()-stateStartTime)>5000){
			current_state=YELLOW;
			previous_state=GREEN;
			stateStartTime=HAL_GetTick();
		}
